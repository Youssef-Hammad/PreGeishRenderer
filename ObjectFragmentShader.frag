...
vec3 calculateDirectionalLight(DirectionalLight light, vec3 normal, vec3 viewDir)
{
	...
	ambient = light.ambient*vec3(texture(material.diffuse,TexCoords)).rgb;
	...
	diffuse = light.diffuse*diffAngle*vec3(texture(material.diffuse,TexCoords)).rgb;
	...
	specular = light.specular*specularAngle*vec3(texture(material.specular,TexCoords)).rgb;
